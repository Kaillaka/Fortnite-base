...
//		if (!win32u) win32u = user32; // Fallback
//
//		LPVOID ntusersendinput_addr = (LPVOID)GetProcAddress(user32, "NtUserSendInput");
//		if (!ntusersendinput_addr)
//			ntusersendinput_addr = (LPVOID)GetProcAddress(win32u, "NtUserSendInput");
//
...
//	}
//
//	static NTSTATUS WINAPI spoofed_call(UINT cinputs, LPINPUT pinputs, int cbsize)
//	{
//		LPVOID exec_mem = VirtualAlloc(0, 0x1000, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
//		if (!exec_mem) return 0;
//
//		memcpy(exec_mem, ntusersendinput_bytes, 30);
//		NTSTATUS result = reinterpret_cast<NTSTATUS(NTAPI*)(UINT, LPINPUT, int)>(exec_mem)(cinputs, pinputs, cbsize);
//
//		SecureZeroMemory(exec_mem, 0x1000);
...
//		create_window_in_band = reinterpret_cast<CreateWindowInBand>(GetProcAddress(user::user32, "CreateWindowInBand"));
//		if (!create_window_in_band)
//		{
//			create_window_in_band = reinterpret_cast<CreateWindowInBand>(GetProcAddress(user::win32u, "CreateWindowInBand"));
