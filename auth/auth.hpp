...
namespace KLAUTH {
	...
	public:
		...
		KLITH(std::string name, std::string ownerid, std::string version, std::string url, std::string path, bool debugParameter = false)
		...
		static std::string check_section_integrity_failed_message;
		static std::string LockMemAccess_failed_message;
		...
		public:
			std::string secret;
			std::string link;
			Tfa& handleInput(KLAUTH::KLITH& apiInstance);
		private:
			void QrCode();
		};
